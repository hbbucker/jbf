...
public abstract class GenericBaseCad<T, TDao extends DAOGenerico> extends WindowCrud {
	...
	public GenericBaseCad(String urlform, T vo, TDao dao) {
		super(urlform);
			...
			Messagebox.show(Labels.getRequiredLabel("button.delete.question"), Labels.getRequiredLabel("system.warning") + "!!!", Messagebox.YES | Messagebox.NO, Messagebox.QUESTION, new EventListener<Event>() {
				...
				public void onEvent(Event arg0) throws Exception {
					if (arg0.getName().equals("onYes")) {
						Integer qtdreg = 0;
						try {
							qtdreg = dao.delReg(VoUtils.getPrimaryKey(GenericBaseCad.this.vo));
						} catch (SQLException e) {
							e.printStackTrace();
						}
						if (qtdreg > 0) {
							if (testImplementsCallback()) {
								((CallbackCrud) GenericBaseCad.this).onApagarSuccess(GenericBaseCad.this.vo);
							} else {
							...
							if (testImplementsCallback()) {
								((CallbackCrud) GenericBaseCad.this).onApagarFailed(new Exception(Labels.getRequiredLabel("register.fail.delete")));
							} else
		...
		try {
			if (this.validarForm() && this.trataVO() && this.validaFormExt()) {
				if (this.dao.updRegByCod(this.vo) > 0) {
					if (this.abertoPeloMenu) {
						if (testImplementsCallback()) {
							((CallbackCrud) GenericBaseCad.this).onSalvarSuccess(this.vo);
						} else
					...
					if (testImplementsCallback()) {
						((CallbackCrud) GenericBaseCad.this).onApagarFailed(new Exception(Labels.getRequiredLabel("register.fail.save")));
					} else
			...
			if (testImplementsCallback()) {
				((CallbackCrud) GenericBaseCad.this).onApagarFailed(e);
			} else
				throw e;
		} catch (Exception e) {
			if (testImplementsCallback()) {
